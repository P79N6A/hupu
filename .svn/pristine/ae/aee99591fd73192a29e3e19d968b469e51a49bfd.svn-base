...
//            ->field('svg.id,svg.vip_name,count(svg.id) as cnt')
    ...
    public function getAgentList()
    {
        ...
        $param = input('is_give');
        if ('' != $param) {
            $condition[] = ['G.is_give','=',$param];
        }else{
            $condition[] = ['G.is_give','=',0];
        ...
        $data = $model
            ...
            ->join("vip_group E ", "E.id=A.vip_group_id", "left")
            ->join("vip_list F ", "F.id=A.vip_id", "left")
            ->join("user_detail H ", " H.user_id=A.rec_user_id",'left')
            ->join("capital_log G ", "G.object_id=A.user_id")
            ->where($condition)
            ->field('A.*, B.region_name as province_name, C.region_name as city_name, D.region_name as district_name, E.vip_name, F.money as v_money, H.nick_name as rec_name')
