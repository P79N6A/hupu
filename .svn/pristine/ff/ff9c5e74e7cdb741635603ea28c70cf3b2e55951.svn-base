...
class Share extends ApiBase
{
    ...
    public function create_share()
    {
        ...
        $activity_id = $this->share_model->where(['id'=>$share_id])->column('activity_id');
        ...
        $res = $this->share_model->where(['id'=>$share_id])->update($share_data);
        if (!$res){
            return json(['ResultType'=>self::__ERROR2__,'Message'=>"操作失败"]);
        }else{
        	//保持消费记录表
