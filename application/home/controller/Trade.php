...
class  Trade extends HomeBase
{
    ...
    public function myTrades()
    {
        ...
        $joinTrade = Db::table('s_user_trade_area_member m')->join('s_user_trade_area as a','a.id = m.trade_id', 'left')
    ...
    public function memberManager()
    {
        $userInfo = session('user_info');
        $trade = Db::table('s_user_trade_area')->where(['id' => Input('id')])->find();
        $members = Db::table('s_user_trade_area_member m')->join('s_user_info u','u.id=m.user_id', 'left')
    ...
    public function adManager()
    {
        ...
        if($this->request->isPost()){
            if ($ad) {
                ...
                $res = Db::table('s_user_trade_area_ad')->update($ad);
                if ($res === false) {
    ...
    {
        header("location:".url('Home/Trade/Quotientsearch'));
        if($this->request->isPost()){
            $get = Input("post.");
           // var_dump($get);die;
    ...
    public function editTrade()
    {
        ...
        if($this->request->isPost()){
            ...
            $res = Db::table('s_user_trade_area')->update($trade);
            if ($res === false) {
    ...
    public function message()
    {
        ...
        $messages = Db::table('s_usre_trade_msg m')
            ->join('s_user_trade_area a','a.id=m.trade_id', 'left')
    ...
    public function messageInfo()
    {
        if($this->request->isPost()){
            ...
            Db::table('s_user_trade_area_member')->where(array('trade_id'=>$mesInfo['trade_id'],'user_id'=>$mesInfo['source_id']))->update(array('status'=>$status ));
            Db::table('s_usre_trade_msg')->where(array('id'=>$id))->update(array('status'=>$status == 1 ? 2 : 3,'edit_time'=>time()));
    ...
    public function agree()
    {
        ...
        $member['status'] = 1;
        $res = Db::table('s_user_trade_area_member')->update($member);
        if ($res === false) {
    ...
    public function select()
    {
        if($this->request->isPost()){
          ...
          //  unset($post['type1']);
            ...
            if($channel == 1){
                header("location:".url("Home/Card/Basicinfor"));
            }else if($channel == 2){
