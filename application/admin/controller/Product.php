...
class Product extends Base
{
    ...
    public function index()
    {
        ...
        $lists = Db::name('product_type')->select();
    ...
    public function getList()
    {
        ...
        $param = input('mobile');//类别
        if ('' != $param) {
            $condition[] = ['B.mobile','=',$param];
        }
        ...
        $data = $model
            ->alias("A")
            ->where($condition)
            ->join('user_detail B','B.user_id=A.user_id')
            ->join('product_type C','C.id=A.type_id')
            ->field('A.*,B.nick_name as user_name,B.mobile,C.title as type_name')
            ->order('A.id desc');
